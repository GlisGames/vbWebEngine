...
import type { StyleList, StyleTable } from '@vb/core/vbStyle';
...
export function load_assets(loader: PIXI.Loader, assets: AssetList) {
    ...
    for (const filename of assets.sound) {
        const filename_stripped = filename.split('/')[1];
        loader.add(filename_stripped, filename);
    }
    for (const [fontFamily, filename] of assets.font) {
    ...
    for (const [_code, filename] of Object.entries(assets.lang)) {
        loader.add(filename);
    }
...
export function get_textureMap(loader: PIXI.Loader, assets: AssetList) {
    const textureMap: { [name: string]: PIXI.Texture } = {};
    // get textures from each of the single image
    for (const filename of assets.img) {
        const filename_stripped = filename.split('/')[1];
        const tex = loader.resources[filename].texture;
        if (tex != undefined) {
            textureMap[filename_stripped] = tex;
        }
    }
    // get textures from texture atlas
    for (const filename of assets.img_atlas) {
        const sheet = loader.resources[filename].spritesheet;
        ...
        const frames = <any[]><unknown>sheet.data.frames;
        for (let i = 0; i < frames.length; i++) {
            const frame_filename = frames[i]['filename'];
            const tex = sheet.textures[i];
...
export function get_multipack_sequenceMap(loader: PIXI.Loader, assets: AssetList) {
     ...
     */
    const sequenceMap_tmp: { [name: string]: [string, PIXI.Texture][] } = {};
    ...
    const sequenceMap: { [name: string]: PIXI.Texture[] } = {};
    for (const [key, value] of Object.entries(sequenceMap_tmp)) {
...
export function get_SpineMap(loader: PIXI.Loader, assets: AssetList) {
    const spineMap: { [name: string]: SpineData } = {};
...
export function get_styleMap(loader: PIXI.Loader, assets: AssetList) {
    const styleMap: { [name: string]: StyleTable } = {};
...
export function get_localeMap(loader: PIXI.Loader, assets: AssetList) {
    const localeMap: { [code: string]: LocalizationTable } = {};
    // create localization table
    for (const [code, filename] of Object.entries(assets.lang)) {
        const table = <Omit<LocalizationTable, "code">>loader.resources[filename].data
        localeMap[code] = {
            code: code,
            ...table
        };
